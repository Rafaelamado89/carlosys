...
export const Route = createFileRoute("/_authenticated/work-orders/new")({
  head: () => ({ meta: [{ title: "New work order · Workshop ERP" }] }),
...
function NewWorkOrder() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto">
      ...
      <h1 className="text-3xl font-bold tracking-tight mb-6">New work order</h1>

      <form onSubmit={submit} className="bg-card border rounded-xl p-6 space-y-5">
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Client name" required value={form.client_name} onChange={(v) => setForm({ ...form, client_name: v })} />
          <Field label="Phone" value={form.client_phone} onChange={(v) => setForm({ ...form, client_phone: v })} />
          <Field label="Make" required value={form.motorcycle_make} onChange={(v) => setForm({ ...form, motorcycle_make: v })} />
          <Field label="Model" required value={form.motorcycle_model} onChange={(v) => setForm({ ...form, motorcycle_model: v })} />
        ...
        <div>
          <label className="text-sm font-medium mb-1.5 block">Notes / diagnosis</label>
          <textarea
            value={form.notes}
        ...
        <div className="flex gap-3">
          ...
          <Link to="/work-orders" className="px-5 py-2.5 rounded-md border font-medium hover:bg-muted">Cancel</Link>
        </div>
      </form>
    </div>
  );
